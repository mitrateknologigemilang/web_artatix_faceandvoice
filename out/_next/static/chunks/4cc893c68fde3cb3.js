...
var best_index : i32 = 0;`,`if (${e.getByIndices("input_indices")} ${r.selectLastIndex>0?">=":">"} value) {
    ...
    `,rR=(e,r,i,a,n,s,o,u,l,d,p,c)=>{var h,f,m,g,y,_,b,$,v,w,x,S,k,T,I,C,E,z,A,O,M,R,N,B,D;let U,P,L,q,W,V,j,G,H,F,K,Z,Q,X,Y,J,ee,et,er,ei,ea,en,es,eo,eu,el,ed,ep,ec,eh,ef,em,eg,ey=Math.min(e.outputCount,1+ +!!o+ +!!u),e_=ey>1?o:void 0,eb=ey>1?u:void 0,e$=ey>1?d.pastSequenceLength:0,ev=e$+d.kvSequenceLength,ew=l&&tc.size(l.dims)>0?l:void 0,ex=[r,i];e_&&tc.size(e_.dims)>0&&ex.push(e_),ew&&ex.push(ew),p&&ex.push(p),c&&ex.push(c);let eS=e.compute((h=ey,f=r,m=i,g=e_,y=ew,_=d,b=e$,$=p,v=c,U=b+_.kvSequenceLength,P=[_.batchSize,_.numHeads,_.sequenceLength,U],L=h>1&&g,q=_.kvNumHeads?_.kvNumHeads:_.numHeads,W=L?[_.batchSize,q,U,_.headSize]:void 0,V=_.nReps?_.nReps:1,j=0===_.scale?1/Math.sqrt(_.headSize):_.scale,G=tH(_.headSize),H=_.headSize/G,F={x:Math.ceil(U/12),y:Math.ceil(_.sequenceLength/12),z:_.batchSize*_.numHeads},K=[{type:12,data:_.sequenceLength},{type:12,data:H},{type:12,data:U},{type:12,data:_.numHeads},{type:12,data:_.headSize},{type:1,data:j},{type:12,data:b},{type:12,data:_.kvSequenceLength},{type:12,data:V}],Z=L&&g&&tc.size(g.dims)>0,Q=["type","type"],Z&&Q.push("type"),y&&Q.push("type"),$&&Q.push("type"),v&&Q.push("type"),X=[{dims:P,dataType:f.dataType,gpuDataType:0}],L&&X.push({dims:W,dataType:f.dataType,gpuDataType:0}),{name:"AttentionProbs",shaderCache:{hint:`${G};${void 0!==y};${void 0!==g};${h}`,inputDependencies:Q},getRunData:()=>({outputs:X,dispatchGroup:F,programUniforms:K}),getShaderSource:e=>{let r=tY("q",f.dataType,f.dims,G),i=[r,tY("key",m.dataType,m.dims,G)];if(Z){let e=tY("past_key",g.dataType,g.dims,G);i.push(e)}y&&i.push(tY("attention_bias",y.dataType,y.dims));let a=$?tY("seq_lens",$.dataType,$.dims):void 0;a&&i.push(a);let n=v?tY("total_sequence_length_input",v.dataType,v.dims):void 0;n&&i.push(n);let s=tJ("output",f.dataType,P),o=[s];L&&o.push(tJ("present_key",f.dataType,W,G));let u=tj(1,G);return`
  const TILE_SIZE = 12u;
  ...
  }`,getRunData:()=>({outputs:[{dims:e[0].dims,dataType:e[0].dataType}],dispatchGroup:{x:Math.ceil(l/64)},programUniforms:a?[{type:12,data:l},...tG(s)]:[{type:12,data:l}]})}})(a,s))}}),nQ=es(()=>{nB(),nV(),rD=e=>{var r;let i,a,n,s,o,u,l,d;(e=>{if(3!==e[0].dims.length)throw Error("input should have 3 dimensions");if(![320,640,1280].includes(e[0].dims[2]))throw Error("number of channels should be 320, 640 or 1280");if(1!==e[1].dims.length)throw Error("bias is expected to have 1 dimensions");if(e[0].dims[2]!==e[1].dims[0])throw Error("last dimension of input and bias are not the same")})(e.inputs),e.compute((i=(r=e.inputs)[0].dims,a=r[0].dims[2],n=tc.size(i)/4,s=r[0].dataType,o=tY("input",s,i,4),u=tY("bias",s,[a],4),l=tY("residual",s,i,4),d=tJ("output",s,i,4),{name:"BiasAdd",getRunData:()=>({outputs:[{dims:i,dataType:r[0].dataType}],dispatchGroup:{x:Math.ceil(n/64)}}),getShaderSource:e=>`
...
`),r.cacheKey,e.inputs[0].dataType))}}),nY=es(()=>{nB(),nV(),nX(),i_=e=>{var r;let i,a,n,s,o,u;(e=>{if(3!==e[0].dims.length)throw Error("input should have 3 dimensions");if(![2560,5120,10240].includes(e[0].dims[2]))throw Error("hidden state should be 2560, 5120 or 10240");if(1!==e[1].dims.length)throw Error("bias is expected to have 1 dimensions");if(e[0].dims[2]!==e[1].dims[0])throw Error("last dimension of input and bias are not the same")})(e.inputs),e.compute(((i=(r=e.inputs)[0].dims.slice())[2]=i[2]/2,a=tY("input",r[0].dataType,r[0].dims,4),n=tY("bias",r[0].dataType,[r[0].dims[2]],4),s=tJ("output",r[0].dataType,i,4),o=tc.size(i)/4,u=tV(r[0].dataType),{name:"BiasSplitGelu",getRunData:()=>({outputs:[{dims:i,dataType:r[0].dataType}],dispatchGroup:{x:Math.ceil(o/64)}}),getShaderSource:e=>`
...
`)};
    ...
    ${e.mainStart()}
    ${e.guardAgainstOutOfBoundsWorkgroupSizes("uniforms.vec_size")}
    ${a}`},getRunData:()=>({outputs:[{dims:s,dataType:r[0].dataType}],dispatchGroup:{x:Math.ceil(p/64)},programUniforms:c})}),{inputs:[0]})}}),ss=es(()=>{nM(),nB(),nV(),nX(),am=e=>{var r;let i,a,n,s;e.inputs.length<2||0===tc.size(e.inputs[1].dims)?ic(e):e.compute((i=(r=e.inputs)[0].dataType,a=tc.size(r[0].dims),s=(n=tc.size(r[1].dims))%4==0,{name:"FastGeluWithBias",shaderCache:{hint:`${s}`,inputDependencies:["type","type"]},getShaderSource:e=>{let r=tY("x",i,[1],4),a=tY("bias",i,[1],4),n=tJ("y",i,[1],4),o=e=>`
